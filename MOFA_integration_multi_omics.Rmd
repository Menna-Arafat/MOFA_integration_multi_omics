...
data_dir <- "C:/Users/USER/github_projects/MOFA_integration_multiple_omics/data/"
list.files(data_dir)
...
#               x = x %>% filter(!is.na(gene_symbol)& gene_symbol != "") %>% as.data.frame() %>% 
#               #' ## drop duplicate gene symbols, keeping the greater of them
#               mutate(rs = rowSums(dplyr::select(., where(is.numeric)), na.rm = TRUE))/ncol(x) %>%
...
## check
```{r}
#duplicated(TNBC_data[["mRNA"]]$gene_symbol) %>% table()
```
...
```{r}
# select_most_variable <- function(data, top_n = 5000) {
...
## Apply this to each dataframe in TNBC_shared

```{r}
# TNBC_shared_most_variable <- lapply(TNBC_shared, function(x){
#   if(nrow(x) >5000){
#   x= select_most_variable(x, top_n = 5000)
#   }else{
#     return(x)
#   }
#   })
...
MOFAobject <- run_mofa(MOFAobject, outfile= "MOFA2_TNBC.last.hdfs", use_basilisk = TRUE)

saveRDS(MOFAobject,"output/MOFA2_TNBC.last.rds")
```
## ------------------------------------------------------------------------------------------------------
